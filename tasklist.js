...
//setting popup listner and add task listener at starting
function init() {
  ...
  if (taskList.length == 0)
    showMsg(document.getElementById("empty-msg"), "No Tasks Available");
  //check for the tasks
...
var taskList = []; //global arry of tasks
var totalTasks = 0; // global var of total task

//comman function for edit and add task
function submitButttonListener(taskNameElement, taskDetailElement) {
  ...
  submit.onclick = function() {
    ...
    if (taskName.value == "" || taskDetail.value == "") {
      showMsg(
        document.getElementById("fields-check-msg"),
...
//function to populate tasks in task container
function populateTasks(taskNameValue, taskDetailValue, onReload, id) {
  let taskId = onReload ? id : totalTasks;
  let divToBeAppended =
    '<div class="task"><div class="message-symbol"><i class="fas fa-comments"></i></div><div class="message-discription"><div class="title" id="' +
    taskId +
    '">' +
    taskNameValue +
    '</div><div class="discription">' +
    taskDetailValue +
    '</div><div class="edit-task"><span class="edit"> edit </span> <span class="delete">delete</span></div></div></div>';
...
//edit and delete functionality for task
